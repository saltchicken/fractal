...
void generate_points_batch(long long points_already_generated);
...
void generate_fractal(GLuint pointShaderProgram, GLuint vao, GLuint vbo) {
	...
	while(points_generated < TOTAL_POINTS) {
		generate_points_batch(points_generated);
	...
	std::cout << "\nGeneration complete in " << duration.count() << " seconds." << std::endl;
